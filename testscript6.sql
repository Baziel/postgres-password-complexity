...
\echo 
\echo '==> Now see if we can get it too just not work because the username as password:'
DO
$do$
BEGIN
  EXECUTE format($$CREATE USER bad_pw_user WITH PASSWORD 'Xbad_pw_user123!' VALID UNTIL %L$$, NOW() + INTERVAL '10 minutes');
END;
$do$;

GRANT pci_admin_users TO bad_pw_user;
GRANT ALL ON DATABASE pwdb TO bad_pw_user;
DO
$do$
BEGIN
  EXECUTE format($$ALTER ROLE bad_pw_user VALID UNTIL %L$$, NOW() + INTERVAL '29 days');
END;
$do$;
SELECT pg_sleep(1);
